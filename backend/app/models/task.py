...
class Task(Base):
    ...
    auto_submit: Mapped[bool] = mapped_column(Boolean, default=True)   # 旧的一次性任务是否自动提交订单

    # 调度与人工确认配置
    schedule_mode: Mapped[str] = mapped_column(String(20), default="once")
    daily_start_time: Mapped[Optional[str]] = mapped_column(String(5), nullable=True)
    date_strategy: Mapped[str] = mapped_column(String(20), default="fixed")
    date_offset_days: Mapped[int] = mapped_column(Integer, default=0)
    confirmation_required: Mapped[bool] = mapped_column(Boolean, default=False)
    last_daily_run_date: Mapped[Optional[str]] = mapped_column(String(10), nullable=True)
    last_daily_success_date: Mapped[Optional[str]] = mapped_column(String(10), nullable=True)
    ...
    confirmations = relationship(
        "TicketConfirmation", back_populates="task", cascade="all, delete-orphan"
    )
